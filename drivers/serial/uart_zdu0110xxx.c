...
struct uart_zdu0110xxx_parent_data {
	/* uart_driver_data needs to be first */
	struct uart_driver_config data;
	
	/* This needs to be second (and in the port data) */
	bool is_port;
...
struct uart_zdu0110xxx_port_data {
	...
	/* This needs to be second (and in the parent data) */
	bool is_port;

	/* Reference to the parent */
	const struct device *parent;

	uint_8_t uart_number;
	uint8_t uart_enable;
...
static int uart_zdu0110xxx_poll_in(const struct device *dev, unsigned char *c)
{
	...
	const struct uart_zdu0110xxx_port_data *data = 
			(const struct uart_zdu0110xxx_port_data *)(dev-data);
	const struct device *parent = data->parent;
	if (!parent) {
		return -EINVAL;
	}
	
	const struct uart_zdu0110xxx_parent_data *parent_data =
			(const struct uart_zdu0110xxx_parent_data *)(parent->data);
...
{
    const struct uart_zdu0110xxx_drv_config *config = dev->config;
    const struct uart_zdu0110xxx_dev_data_t *data = 
    		(const struct uart_zdu0110xxx_dev_data_t *)dev->data;
    uint8_t uart_number = data->uart_number;
    uint8_t cmd[16];
    size_t cmd_len;
	...
	cmd_len = 0;
	cmd[cmd_len++] = ZDU_CMD_UART_GET_STATUS(uart_number)
	ret = zdu0110xxx_send_command(dev, cmd, cmd_len, &data->uart_status, 1);
...
static void uart_zdu0110xxx_interrupt_worker(struct k_work *work)
{
    ...
    for (index = 0; index < data->child_count; index++) {
		...
		const struct uart_zdu0110xxx_port_data *port_data = 
			(const struct uart_zdu0110xxx_port_data *)(child->data);
...
static int uart_zdu0110xxx_init_parent(const struct device *dev)
{
	int err;
	const struct uart_zdu0110xxx_drv_config *config = dev->config;
	const struct uart_zdu0110xxx_parent_data *data = 
			(const struct uart_zdu0110xxx_parent *)(dev->data);
			
	if (dev->is_port) {
		return uart_zdu0110xxx_init(dev);
	}
...
static int zdu0110xxx_set_baud_rate(const struct device *dev, uint32_t baudrate)
{
	const struct uart_zdu0110xxx_drv_config *config = dev->config;
	const struct uart_zdu0110xxx_port_data *data = 
			(const struct uart_zdu0110xxx_port *)(dev->data);
	...
	if ((baud_percent_variance > 3) || (baud_percent_variance < -3)) {
		LOG_ERR("ZDU0110[0x%X]: requested baud rate (%d) not attainable,"
			...
			i2c_addr, baudrate, actual_baud, baud_percent_variance);
		return -EINVAL;
	}

	config->baudrate[uart_number] = actual_baud;
...
static int uart_zdu0110xxx_init(const struct device *dev)
{
	int err;
	const struct uart_zdu0110xxx_drv_config *config = dev->config;
	const struct uart_zdu0110xxx_port_data *data = 
	...
	if (!dev->is_port) {
		return uart_zdu0110xxx_init_parent(dev);
	}
	...
	requested_baud = config->baudrate[uart_number];
	ret = zdu0110xxx_set_baud_rate(dev, requested_baud);
			     ...
			     uint32_t *val)
{
	...
	if (ctrl == UART_LINE_CTRL_BAUD_RATE) {	
		*val = config->baudrate[uart_number];
		return 0;
	}
...
#define UART_ZDU0110XXX_PORT_DEVICE(t, parent_id, port_number)				\
	static struct uart_zdu0110xxx_port_data									\
			uart_zdu0110##t##_##parent_id##_port_##port_id##_data = {		\
		.is_port = true,													\
		.parent = uart_zdu0110##t##_##parent_id,							\
		.uart_number = port_id, 											\
		.uart_enable = 0x00,												\
	IF_ENABLED(CONFIG_GPIO_ZDU0110XXX_INTERRUPT, (							\
	    ...
	    .cb = NULL, 														\
																			...
																			\
	DEVICE_INIT(uart_zdu0110##t##_##parent_id##_port_##port_id,				\
			    DT_INST_LABEL(DT_INST_PORT(id, t)),							\
			    &uart_zdu0110xxx_init,										\
			    &uart_zdu0110##t##_##parent_id##_port_##port_id##_data,		\
			    &uart_zdu0110##t##_##parent_id##_cfg,						\
			    POST_KERNEL,												\
...
#define UART_ZDU0110XXX_PARENT_DEVICE(t, id, uartcount)						\
	static const struct uart_zdu0110xxx_drv_config							\
		...
		.baudrate = { DT_FOREACH_CHILD(DT_INST_MASTER(id, t),				\
		                             GET_CURRENT_SPEED_COMMA) },			\
	...
	DEVICE_INIT(uart_zdu0110##t##_##id,										\
			    DT_INST_LABEL(DT_INST_MASTER(id, t)),						\
			    &uart_zdu0110xxx_init_parent,								\
			    &uart_zdu0110##t##_##id##_data,								\
			    &uart_zdu0110##t##_##id##_cfg,								\
			    POST_KERNEL,												\
...
#define UART_ZDU0110RFX_PARENT_INSTANCE(id)	\
	GPIO_ZDU0110XXX_PARENT_DEVICE(rfx, id, 1)
#define UART_ZDU0110RFX_PORT_INSTANCE(id)   \
	GPIO_ZDU0110XXX_PORT_DEVICE(rfx, DT_PARENT(id), DT_PROP(id, port_number))

#define DT_DRV_COMPAT zilog_zdu0110rfx
DT_INST_FOREACH_STATUS_OKAY(GPIO_ZDU0110RFX_PARENT_INSTANCE)
#undef DT_DRV_COMPAT

#define DT_DRV_COMPAT zilog_zdu0110rfx_port
DT_INST_FOREACH_STATUS_OKAY(GPIO_ZDU0110RFX_PORT_INSTANCE)
#undef DT_DRV_COMPAT
...
#define UART_ZDU0110RHX_PARENT_INSTANCE(id)	\
	GPIO_ZDU0110XXX_PARENT_DEVICE(rhx, id, 1)
#define UART_ZDU0110RHX_PORT_INSTANCE(id)   \
	GPIO_ZDU0110XXX_PORT_DEVICE(rhx, DT_PARENT(id), DT_PROP(id, port_number))

#define DT_DRV_COMPAT zilog_zdu0110rhx
DT_INST_FOREACH_STATUS_OKAY(GPIO_ZDU0110RHX_PARENT_INSTANCE)
#undef DT_DRV_COMPAT

#define DT_DRV_COMPAT zilog_zdu0110rhx_port
DT_INST_FOREACH_STATUS_OKAY(GPIO_ZDU0110RHX_PORT_INSTANCE)
#undef DT_DRV_COMPAT
...
#define UART_ZDU0110RJX_PARENT_INSTANCE(id)	\
	GPIO_ZDU0110XXX_PARENT_DEVICE(rjx, id, 1)
#define UART_ZDU0110RJX_PORT_INSTANCE(id)   \
	GPIO_ZDU0110XXX_PORT_DEVICE(rjx, DT_PARENT(id), DT_PROP(id, port_number))

#define DT_DRV_COMPAT zilog_zdu0110rjx
DT_INST_FOREACH_STATUS_OKAY(GPIO_ZDU0110RJX_PARENT_INSTANCE)
#undef DT_DRV_COMPAT

#define DT_DRV_COMPAT zilog_zdu0110rjx_port
DT_INST_FOREACH_STATUS_OKAY(GPIO_ZDU0110RJX_PORT_INSTANCE)
#undef DT_DRV_COMPAT
...
#define UART_ZDU0110QUX_PARENT_INSTANCE(id)	\
	GPIO_ZDU0110XXX_PARENT_DEVICE(qux, id, 1)
#define UART_ZDU0110QUX_PORT_INSTANCE(id)   \
	GPIO_ZDU0110XXX_PORT_DEVICE(qux, DT_PARENT(id), DT_PROP(id, port_number))

#define DT_DRV_COMPAT zilog_zdu0110qux
DT_INST_FOREACH_STATUS_OKAY(GPIO_ZDU0110QUX_PARENT_INSTANCE)
#undef DT_DRV_COMPAT

#define DT_DRV_COMPAT zilog_zdu0110qux_port
DT_INST_FOREACH_STATUS_OKAY(GPIO_ZDU0110QUX_PORT_INSTANCE)
#undef DT_DRV_COMPAT
