...
{
	uint8_t buf[CONFIG_RAM_SHELL_BUFFER_SIZE];
	const struct device *ram;
	off_t offset;
	size_t len;
	int err;
	...
	ram = device_get_binding(argv[args_indx.device]);
	if (!ram) {
		shell_error(shell, "RAM device not found");
		return -EINVAL;
	}

	shell_print(shell, "Reading %d bytes from RAM, offset %d...", len,
		    offset);

	err = ram_read(ram, offset, buf, len);
	if (err) {
		shell_error(shell, "RAM read failed (err %d)", err);
		return err;
	}
...
{
	uint8_t wr_buf[CONFIG_RAM_SHELL_BUFFER_SIZE];
	uint8_t rd_buf[CONFIG_RAM_SHELL_BUFFER_SIZE];
	const struct device *ram;
	unsigned long byte;
	off_t offset;
	size_t len;
	...
	shell_print(shell, "Writing %d bytes to RAM...", len);

	err = ram_write(ram, offset, wr_buf, len);
	if (err) {
		shell_error(shell, "RAM write failed (err %d)", err);
		return err;
	}

	shell_print(shell, "Verifying...");

	err = ram_read(ram, offset, rd_buf, len);
	if (err) {
		shell_error(shell, "RAM read failed (err %d)", err);
		return err;
	}
...
static int cmd_size(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "%d bytes", ram_get_size(ram));
	return 0;
}
...
static int cmd_fill(const struct shell *shell, size_t argc, char **argv)
{
	...
	shell_print(shell, "Writing %d bytes of 0x%02x to RAM...", len,
		    pattern);
...
SHELL_STATIC_SUBCMD_SET_CREATE(ram_cmds,
	...
	SHELL_CMD_ARG(write, NULL,
		      ...
		      4, CONFIG_RAM_SHELL_BUFFER_SIZE - 1),
...
SHELL_CMD_REGISTER(ram, &ram_cmds, "RAM shell commands", NULL);
